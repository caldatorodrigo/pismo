...
public class TransactionService implements TransactionServiceInterface {
	...
	private ApiError validFields(TransactionsDTO transactionDto) {
		...
		/** Check if filds is null ou Blank */
		if (transactionDto.getAccount_ID() == null || transactionDto.getAccount_ID() == 0) {
			errorList.add("006 :Transaction field Account is mandatory!");
		} else {
		...
		if (transactionDto.getOperationType_ID() == null || transactionDto.getOperationType_ID() == 0) {
			errorList.add("007 :Transaction field OperationType_ID is mandatory!");
		} else {
			...
			if (operationsTypesModel == null) {
				errorList
						.add("002: Operation Type not found - Possibly type of operation not supported by the system.");
			}
		}

		if (transactionDto.getAmount() == null || transactionDto.getAmount() == BigDecimal.ZERO) {
			errorList.add("008 :Transaction field OperationType_ID is mandatory!");
		} else {
