...
public class AccountsService implements AccountsServiceInterface {
	...
	private ApiError validFields(AccountsDTO accountDto) {
		...
		if(accountDto.getDocument_Number() == null || accountDto.getDocument_Number() == "") {
			errorList.add("004 :Account field Document_Number is mandatory!");
		}
		...
		if (AccountsModelList.size() > 0) {
			errorList.add("005 :Account found - possibly the account has already been created.");
		}
