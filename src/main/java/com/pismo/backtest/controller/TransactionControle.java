...
public class TransactionControle {
    ...
    @Autowired
    public TransactionControle(TransactionsRepository transactionsRepository, AccountsRepository accountsRepository,
	...
	private ApiError validFields(TransactionsDTO transactionDto) {
        ...
        Optional<OperationsTypesModel> operationsTypesModel = operationsTypesRepository.findById(transactionDto.getOperationType_ID());
        ...
        Boolean valid = validAmount(Long.valueOf(transactionDto.getOperationType_ID()).intValue(), transactionDto.getAmount());
        if(!valid) {
        	errorList.add("003: Amount outside the standard of the operation types - check the amount rules by type of operation.");
        }
        
        if(errorList.size() > 0) {
        	return new ApiError(HttpStatus.BAD_REQUEST, "Error encountered while processing the transaction", errorList);
        }else {
        	return null;
        }
	}
